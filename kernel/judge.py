...
def _judge_run(model, sys_prompt, user, effort=None, judge=None, tier="triage", mark=None):
    ...
    if engine != "codex":
        # Claude-engine only: usage.json is the CLAUDE account's windows — gating codex
        # calls on it would be wrong in both directions (codex limits surface per call
        # as error replies instead). docs/codex.md.
        try:
...
def _plan_session(fsid, path, now):
    ...
    save_goals(fsid, store)
    return placed
_hidden_lkg = [None]                     # (stat generation, flags) last-known-good —
#                                          generation-keyed since r58 P1.4
...
def _hidden_from_feed(fsid):
    ...
    A fault serves the last-known-good copy ONLY while the file's stat generation still
    matches the read it came from (r58 P1.4, reproduced: an EIO on a NEWER inode let a
    stale un-muted copy re-enter a freshly muted session into planning); otherwise, and
    with no copy at all, fail CLOSED — hidden (r57 P1.4)."""
    p = STATE / "session-flags.json"
    try:
        st = p.stat()
    ...
    try:
        ...
        _hidden_lkg[0] = (key, d)                    # generation-keyed copy (r58 P1.4)
        f = d.get(fsid)
        ...
        # an unreadable window must not UN-mute a session (the r57 audit's P1.4) — the
        # copy answers only for the SAME generation; a moved world or no copy fails
        # CLOSED, said OUT LOUD once (r57 wave 2: the silent hold read as a planner that
        # just stopped, with nothing anywhere naming the corrupt file).
        hit = _hidden_lkg[0]
        if isinstance(hit, tuple) and key is not None and hit[0] == key:
            f = hit[1].get(fsid)
            return bool(isinstance(f, dict) and f.get("hideFromFeed"))
        if not _hidden_warned[0]:
            _hidden_warned[0] = True
            sys.stderr.write("judge: session-flags is unreadable (or moved past its last "
                             "good read) — holding every session out of planning (fail "
                             "closed) until a readable copy appears\n")
        return True
